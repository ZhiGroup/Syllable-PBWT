...
int M, N, n, Q;
double L;
vector<vector<unsigned long long>> x, h, z;
...
int main(int argc, char** argv) {
	...
	x.resize(M + 1, vector<unsigned long long>(n));
	...
	while (rem != 0) {
		...
		if (n > 0) minL = max(minL, locs[locs.size() - 1u] - locs[(n - 1) * 64]);
	...
	for (int q = 0; q < Q; q++) {
	ofstream out("/home/vwang1/PBWT_compression/query64-" + to_string(q) + ".txt");
	z[n - 1][q] <<= n * 64 - N;
	for (int i = 0, ip1 = 1; i < n; i++, ip1++) {
		x[M][i] = z[i][q];
		hz[ip1] = ((unsigned __int128) hz[i] * xp[1] + x[M][i]) % MOD;
	}
	...
	for (int i = 0, ip1 = 1, t = 0, req_idx = 0, up_idx = -1, dn_idx = -1, up_ct = 0, dn_ct = 0; ip1 < n; i++, ip1++) {
		...
		double loc = locs[ip1 == n - 1 ? N - 1 : (ip1 + 1) * 64 - 2];
		while (req_idx < i && loc - locs[req_idx * 64] >= L) req_idx++;
		...
		if (up_ct > 0 || touch) {
			...
			while (d[ip1][p] <= req_idx || touch) {
				...
				if (up_idx > -1) {
					...
					while (lo < hi) {
						g = (lo + hi) >> 1;
						if ((x[M][up_idx] & (-1ull >> g)) == (x[above][up_idx] & (-1ull >> g))) {
							hi = g;
						} else {
							lo = g + 1;
						}
		...
		if (dn_ct > 0 || touch) {
			...
			while ((p < M && d[ip1][p] <= req_idx) || touch) {
				...
				if (dn_idx > -1) {
					...
					while (lo < hi) {
						g = (lo + hi) >> 1;
						if ((x[M][dn_idx] & (-1ull >> g)) == (x[below][dn_idx] & (-1ull >> g))) {
							hi = g;
						} else {
							lo = g + 1;
						}
